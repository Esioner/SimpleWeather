        ...
        if (!TextUtils.isEmpty(response)) {
            try {
                ...
                for (int i = 0; i < allCountries.length(); i++) {
                    JSONObject countryObjects = allCountries.getJSONObject(i);
                    Country country = new Country();
                    country.setCityId(cityId);
                    country.setCountryName(countryObjects.getString("name"));
                    country.setWeatherId(countryObjects.getString("weather_id"));
                    country.save();
                }
                return true;
